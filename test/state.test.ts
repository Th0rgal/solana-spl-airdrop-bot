...
test("computeNextDelayMs returns zero when no previous round exists", () => {
  assert.equal(computeNextDelayMs(undefined, 3_600_000, 10_000), 0);
});

test("computeNextDelayMs returns remaining wait time", () => {
  const delay = computeNextDelayMs(10_000, 3_600_000, 20_000);
  assert.equal(delay, 3_590_000);
});

test("computeNextDelayMs never returns negative values", () => {
  const delay = computeNextDelayMs(10_000, 1_000, 20_000);
...
test("saveState persists and loadState restores values", async () => {
  ...
  try {
    await saveState(filePath, { lastRoundCompletedAtMs: 123_456 });
    const state = await loadState(filePath);
    assert.deepEqual(state, { lastRoundCompletedAtMs: 123_456 });
  } finally {
